...
def main(spark, data_file, val_file, model_file):
    ...
    #train_data.show()
   # train_data1=spark.sql('select * from train_data WHERE count > 5')
    train_data2= train_data.withColumn("logcount", log(train_data["count"]))
    val_data2=val_data.withColumn("logcount", log(val_data["count"]))
    ...
    for r in ranks :
        for a in alphas :
            for rp in regularizations :
                ...
                als = ALS(maxIter = 5 , regParam= rp, userCol= "user" , itemCol= "track" , ratingCol ="logcount" , implicitPrefs=True , coldStartStrategy="drop" , alpha= a , rank = r)

                pipeline = Pipeline(stages=[indexer_user_id, indexer_track_id, als])
                
                model = pipeline.fit(train_data2)
                
                predictions = model.transform(val_data2)
